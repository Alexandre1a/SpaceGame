...
def loadSettings():
    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            if settings.get("fps") not in AVAILABLE_FPS:
                print(f"[Settings] FPS invalide ({settings.get('fps')}), utilisation de la valeur par défaut")
                settings["fps"] = DEFAULT_SETTINGS["fps"]
            
            if tuple(settings.get("resolution", ())) not in AVAILABLE_RESOLUTIONS:
                print(f"[Settings] Résolution invalide ({settings.get('resolution')}), utilisation de la valeur par défaut")
    ...
    print(f"[Settings] Aucun fichier de paramètres trouvé, utilisation des valeurs par défaut")
    return DEFAULT_SETTINGS.copy()
